<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<SLTest.Models.VMMenu<SLTest.Models.Tables>>" %>
 ...
 <h2>Столы</h2>
    ...
    <% foreach (var item in Model.items) { %>
        ...
        <tr>
            <td class="btTd">
                 <%: Ajax.ActionLink(" ", "Administration", "Home", new { metName = "Edit", modName = "Tables", id = item.ID },
                                              new AjaxOptions { UpdateTargetId = "aj",
                                                                Url = Url.Action("Edit", "Tables", new { id = item.ID })
    ...
    <ul class="buttons">
     ...
     <p>
       ...
       <%: Html.PageLinks(Model.paginginfo, x => Url.Action("Administration", "Home", new { modName = "Tables", pageNum = x }),
